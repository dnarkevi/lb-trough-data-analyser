# Creative Commons Attribution (CC-BY) 4.0 2023 Domantas Narkevičius.
# Some rights reserved.
#
# For help or bug reports, please contact me via Github:
# https://github.com/dnarkevi/
#
...
# which means you must reference the author. See
# https://creativecommons.org/licenses for more information.
...
def adjacent_averaging(data, window_length):
    """Calculates moving average central exactly like in Origin or LabPlot."""
    n = len(data)
    result = np.empty(n)
    for i in range(n):
...
def export_columns(columns, head, path):
    ...
    """
    file = open(path, 'w', encoding='ascii')
    file.write(head + "\n")
...
class Lbdata():
    ...
    def calc_cm(self):
        if values['-BDIF-']:
            ...
            if values['-fil1-']:
                pi = adjacent_averaging(self.data_pi, win_len)
                ar = self.data_ar
        ...
        if values['-ADIF-']:
            ...
            if values['-fil1-']:
                cm = adjacent_averaging(cm, win_len)
                pi_cmsp = pi
...
filtering_frame = [
    [
         Column([
             [sg.Radio("Adjacent-averaging", "filter", default=True,
                       pad=(5, 5), enable_events=True, k='-fil1-')],
             [sg.Radio("Savitzky-Golay", "filter",
                       pad=(5, 5), enable_events=True, k='-fil2-')],
             [Button("Apply", k='-smooth-')]
         ], element_justification='left', vertical_alignment='top'),
         VSeparator(),
         Column([
             [sg.T("Enable", pad=(5, 5))],
             [sg.T("Window length", pad=(5, 5))],
             [sg.T("Polyorder", pad=(5, 5), k='-TXT_ORD-')],
...
fifth_square = [
    ...
    [
         Frame("Filtering", filtering_frame, pad=(0, 0))
    ],
    [
         Button("Formula", key="-formula1-"),
         ...
         Frame("Plot properties", [[
             Column([
                 [
                     ...
                     sg.Radio("Linear", "dgxiinterp", default=True,
                              pad=(5, 5), k='-interp1-'),
                     sg.Radio("Cubic", "dgxiinterp",
                              pad=(5, 5), k='-interp2-')
                 ],
                 [
...
def main():
    ...
    while True:
        ...
        if event == '-formula1-':
            sg.popup_no_buttons('',
                                title="Equation for compression modulus",
                                background_color='#FFFFFF',
                                icon=r"resources\icon.ico",
                                image=r"resources\formula1.png")
        ...
        if event in {'-fil1-', '-fil2-'}:
            if values['-fil2-']:
                window['-TXT_ORD-'].update(visible=True)
                window['-BDIF_ORD-'].update(visible=True)
                window['-ADIF_ORD-'].update(visible=True)
            else:
                window['-TXT_ORD-'].update(visible=False)
                window['-BDIF_ORD-'].update(visible=False)
                window['-ADIF_ORD-'].update(visible=False)
        ...
        if event == '-formula2-':
            sg.popup_no_buttons('',
                                title="Equation for excess Gibbs free energy",
                                background_color='#FFFFFF',
                                icon=r"resources\icon.ico",
                                image=r"resources\formula2.png")
...
cautious!")
            ...
            pi = np.linspace(0,
                             pi_bound,
                             500)
            ar_comp_A = dgxidata[-1].interpolated_piar(pi)
            ar_comp_B = dgxidata[0].interpolated_piar(pi)
...
if __name__ == "__main__":
    ...
    window = sg.Window("Langmuir–Blodgett trough data analyser", layout,
                       ...
                       size=(1366, 768), icon=r"resources\icon.ico")
    ...
    try:
        ...
        with open('crash_log.txt', 'w', encoding='ascii') as f:
